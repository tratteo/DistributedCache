...
public class VirtualSynchActor extends AbstractActor {
  ...
  private void putInFlushes(int viewId, ActorRef ref){
    if(flushes.containsKey(viewId)){
      flushes.get(viewId).add(ref);
    }else {
      HashSet<ActorRef> set = new HashSet<>();
      set.add(ref);
      flushes.put(viewId, set);
    }
  }

  private boolean isViewFlushed(int index){
    if(flushes.containsKey(index) && proposedView.containsKey(index+1)){
      Set<ActorRef> set = flushes.get(index);
      Set<ActorRef> proposedSet = proposedView.get(index+1);
      return set.size() == proposedSet.size() -1;
    }
    return true;
  }
  ...
  private void onViewChangeMsg(ViewChangeMsg msg) {
    ...
    System.out.println(getSelf().path().name()+ " adding proposed view of size: "+msg.proposedView.size()+", id: "+msg.viewId);
    proposedView.put(msg.viewId, new HashSet<>(msg.proposedView));

    for(ChatMsg uMsg : this.unstableMsgSet){
      multicast(uMsg, msg.proposedView);
    }

    ViewFlushMsg vMsg = new ViewFlushMsg(msg.viewId - 1);
    multicast(vMsg, msg.proposedView);
    ...
    if(!isViewFlushed(msg.viewId-1)) {
      getContext().system().scheduler().scheduleOnce(
              ...
              getSelf(),                                          // destination actor reference
              new FlushTimeoutMsg(msg.viewId - 1),             // the message to send
              getContext().system().dispatcher(),                 // system dispatcher
              getSelf());
    }*/
  ...
  private void onViewFlushMsg(ViewFlushMsg msg) {
    ...
    System.out.println(getSelf().path().name()+" putting in flush "+getSender().path().name());
